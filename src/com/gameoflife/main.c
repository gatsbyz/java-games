      ...
      if (KEY_DOWN_NOW(KEY_A))
        {
          ...
          if (GET_CELL(r,c) == ALIVE || GET_CELL(r,c) == BIRTH)
            GET_CELL(r,c) = DEAD;
          else if (GET_CELL(r,c) == DEAD || GET_CELL(r,c) == DYING)
            GET_CELL(r,c) = ALIVE;
          moveCrosshairs(r, c, r, c);
          flipPage();
          moveCrosshairs(r, c, r, c);
  ...
  for (int i = 0; i < GAME_W * GAME_H; i++)
    {
      ...
      if (c < 2 && gameArr[i] == ALIVE)
        newGameArr[i] = (vMode==NORMAL?DEAD:DYING);
...
//Change Cells to ALIVE or DEAD if dying or birthing
void
applyVerbs()
{
  for(int i = 0; i<GAME_W*GAME_H; i++)
  ...
  if (vMode == NEXT)
    {
      ...
      if (cell == DYING)
        drawImage4(r * CELL_H, c * CELL_W, CELL_W, CELL_H, dying);
    }
}
